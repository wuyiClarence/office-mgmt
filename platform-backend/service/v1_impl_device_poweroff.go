...
func (svc *V1ServiceImpl) DevicePowerOff(c *gin.Context, req *dto.DevicePowerOffReq) error {
	userID, err := utils.GetUserIDFromCtx(c)
	if err != nil {
		return err
	}

	err = sp.CheckPermission(c, enum.ResourceTypeDevice, req.IDs, enum.PermissionKeyPowerOff, userID, utils.IsSuperAdmin(c))
	if err != nil {
		return err
	}
