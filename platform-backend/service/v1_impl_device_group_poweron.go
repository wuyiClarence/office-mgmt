...
func (svc *V1ServiceImpl) DeviceGroupPowerOn(c *gin.Context, req *dto.DeviceGroupPowerOnReq) error {
	userID, err := utils.GetUserIDFromCtx(c)
	if err != nil {
		return err
	}

	err = sp.CheckPermission(c, enum.ResourceTypeDeviceGroup, req.IDs, enum.PermissionKeyPowerOn, userID, utils.IsSuperAdmin(c))
	if err != nil {
		return err
	}
